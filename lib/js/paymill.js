jQuery(document).ready(function () {
	...
	function bridgePreparePayment(){
		// check which payment method is active
			if(jQuery('#form-credit').is(':visible')){
				...
				alert(jQuery('.paymill_amount').val());
				alert(jQuery('.paymill_currency').val());
				...
				paymill.createToken({
					...
					//jQuery(paymill_form_checkout_submit_id).removeAttr("disabled");
						...
						/*	@todo: validate IBAN/BIC
				alert(jQuery('#transaction-form-account').val());
				alert(jQuery('#transaction-form-code').val());
				...
				alert(jQuery('#transaction-form-account').val());
				alert(jQuery('#transaction-form-code').val());
				alert(jQuery('.paymill_holdername').val());
				alert(jQuery('.paymill_amount').val());
				alert(jQuery('.paymill_currency').val());

				paymill.createToken({
					iban:jQuery('#transaction-form-account').val(),
					bic:jQuery('#transaction-form-code').val(),
	...
	jQuery('body').on('change', '#card-number', function() {
		...
		if(cc_type == 'Visa'){
			jQuery('#card-number').show().css("backgroundPosition","0 -240px");
